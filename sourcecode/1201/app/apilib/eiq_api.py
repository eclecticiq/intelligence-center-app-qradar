...
from app.collector.eiq_data import get_verify_ssl_value
...
class EclecticIQ_api(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.verify_ssl = get_verify_ssl_value(verify_ssl)
    ...
    def send_api_request(self, method, path, params=None, data=None):
        ...
        try:
            ...
            self.eiq_logging.exception(
                "Could not perform request to EclecticIQ VA: {0}: {1}. Exception: {2}".format(
                    method, url, e
                )
            )
        ...
        if r and r.status_code in [100, 200, 201, 202, 204]:
            ...
            if r == None:
                msg = "Could not perform request to EclecticIQ VA: {0}: {1}. Check network connectivity.".format(
                    method, url
                )
                self.eiq_logging.exception(msg)
                raise Exception(msg)
                ...
                msg = "Could not perform request to EclecticIQ VA: {0}: {1}. Status code:{2}".format(
            ...
            try:
                ...
                msg = (
                    "EclecticIQ VA returned an error, code:{0}, reason:[{1}], URL: [{2}]"
    ...
    def enable_outgoing_feed(self, outgoing_feed_full_info):
        ...
        self.eiq_logging.info(
            "Enable Outgoing Feed id={0}".format(outgoing_feed_full_info["id"])
        )
    ...
    def get_incoming_feed_blobs_pending(self, feed_id):
        ...
        self.eiq_logging.info(
            "Requesting Incoming feed run status: {0}".format(feed_id)
        )
    ...
    def get_full_feed_info(self, feed_id):
        ...
        self.eiq_logging.info(
            "Requesting full feed info for feed id={0}".format(feed_id)
        )
    ...
    def get_incoming_feed_full_info(self, feed_id):
        ...
        self.eiq_logging.info(
            "Requesting full feed info for incoming feed id={0}".format(feed_id)
        )
    ...
    def get_feed_info(self, feed_ids):
        ...
        for k in feed_ids:
            ...
            try:
                ...
                self.eiq_logging.error(
                    "Feed id={0} information cannot be requested.".format(k)
                )
                continue
            ...
            if not r.json()["data"]:
                ...
                result.append(feed_result)
                self.eiq_logging.debug(
                    "Feed id={0} information retrieved successfully. Received response:".format(
                        k
                    )
    ...
    def get_feed_content_blocks(self, feed, feed_last_run=None):
        self.eiq_logging.debug(
            "Requesting block list for feed id={0}".format(feed["id"])
        )
        ...
        if (
            ...
            r = self.send_api_request(
                ...
                + "{0}/runs/latest".format(feed["id"]),
            )

            data = r.json()["data"]["content_blocks"]
            if feed_last_run["last_ingested"] == data[-1]:
                self.eiq_logging.info(
                    "Received list contains {0} blocks for feed id={1}.".format(
                        len(data), feed["id"]
                    )
                )
                return []
            ...
            self.eiq_logging.debug(
                "Requesting block list for {0} feed.".format(feed["update_strategy"])
            )
            ...
            if (feed["created_at"] != feed_last_run["created_at"]) or feed_last_run[
                ...
                self.eiq_logging.info(
                    ...
                    "reingestion of all the feed content.".format(
                        len(data), feed["update_strategy"], feed["id"]
                    )
                )
                return data
            else:
                try:
                    ...
                    self.eiq_logging.info(
                        "Received list contains {0} blocks for {1} feed:{2}.".format(
                    ...
                    self.eiq_logging.error(
                        "Value of last ingested block not available in Feed {0}.".format(
                            feed["id"]
                        )
                    )
                    return None
            ...
            self.eiq_logging.info(
                "Feed id={0} is running now. Collecting data is not possible.".format(
                    feed["id"]
                )
            )
        else:
            self.eiq_logging.info(
                "Feed id={0} update strategy is not supported. Use Replace or Diff".format(
                    feed["id"]
                )
            )
    ...
    def lookup_observable(self, value, type=None):
        ...
        if observable_response["count"] == 1:
            ...
            self.eiq_logging.info(
                "Finding duplicates for observable:{0}, type:{1}, return first one".format(
                    value, type
                )
            )
            result = {}
    ...
    def get_entity_realtionships(self, source_id=None, target_id=None):
        self.eiq_logging.info(
            "Get realtionshsip for entity. Source id: {0}, Destination id: {1}".format(
                source_id, target_id
            )
        )
        params = {}
    ...
    def search_entity(self, entity_value=None, entity_type=None, observable_value=None):
        ...
        """
        self.eiq_logging.info(
            "Searching Entity:{0} with extracted observable:{1}, type:{2}".format(
    ...
    def create_entity(
        ...
        """
        self.eiq_logging.info(
            "Creating Entity in EclecticIQ Platform. Type:{0}, title:{1}".format(
    ...
    def get_observable(self, observable):
        self.eiq_logging.info(
            "EclecticIQ_api: Searching for Observable: {0}".format(observable)
        )
        path = (
