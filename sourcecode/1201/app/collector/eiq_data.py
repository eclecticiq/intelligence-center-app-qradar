...
def get_proxy(config):
    qpylib.log(str(config), level=LOG_LEVEL_INFO)
...
def get_verify_ssl_value(verify):
    if verify in ["0", "False", "false", False, "", None]:
        return False
    return True
...
class CustomAuth:
    ...
    def __init__(self, config):
        ...
        self._verify = get_verify_ssl_value(config.get(VERIFY))
        self._proxy_ip = config[PROXY_IP]
        self._proxy_username = config[PROXY_USERNAME]
        self._proxy_password = config[PROXY_PASSWORD]
        self._proxy = get_proxy(config)

    @property
    def verify(self):
        """Get value for proxies.
        :return: proxies
        :rtype: dict
        """
        return self._verify
    ...
    def get_eiq_request(self, configs=configs):
        ...
        :rtype: request
        """
        qpylib.log(str(self._proxy), level=LOG_LEVEL_INFO)
        return Request(
            self._host,
            auth=BEARER_TOKEN,
            token=self._api_key,
...
class EIQApi:
    ...
    def create_sighting(
        ...
        sighting[DATA][DATA][SECURITY_CONTROL]["time_start"] = format_time_to_iso(
    ...
    def get_outgoing_feeds_ids_names(feeds):
        ...
        ids_and_names = []
        for data in feeds:
            if (data["content_type"] == "urn:eclecticiq.com:csv-extracts:1.0") and (
                data["transport_type"] == "eiq.outgoing-transports.http-download"
            ):
