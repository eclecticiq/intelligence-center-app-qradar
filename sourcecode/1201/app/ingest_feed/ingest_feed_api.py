...
def ingest_feed(params):
    ...
    for item in feeds_info:
        ...
        qpylib.log(
            "ingest_feed_api.{0}: Checking for availability of necessary RT in Qradar.".format(
                func_name
            ),
            level="info",
        )
        ...
        qpylib.log(
            "ingest_feed_api.{0}: Starting Ingestion of feed #{1}".format(
                func_name, str(item["id"])
            ),
            level="info",
        )
        ...
        for block in blocks:
            qpylib.log(
                "ingest_feed_api.{0}: Feed id={1} preparing data to ingest block {2}.".format(
                    func_name, str(item["id"]), block
                ),
                level="info",
            )
        ...
        qpylib.log(
            "ingest_feed_api.{0}: Feed id={1} was fully ingested/updated.".format(
                func_name, str(item["id"])
            ),
            level="info",
        )
...
def export_csv_to_rs(feed_id, text, qrdar_obj, flag=False):
    ...
    func_name = sys._getframe().f_code.co_name
    qpylib.log(
        "ingest_feed_api.{0}: Exporting to RT feed #{1}".format(func_name, feed_id),
        level="info",
    )
    text = io.StringIO(text)
    csvreader = csv.DictReader(text, delimiter=",")

    data_to_add = {"domain": {}, "ip": {}, "file_hash": {}, "email": {}, "url": {}}
    ...
    if flag:
        ...
        for k in QRADAR_RT_LIST:
            ...
            qpylib.log(
                "ingest_feed_api.{0}: Purging content of RT {1}.".format(
                    func_name, rt_title
                ),
                level="info",
            )
    ...
    if "diff" not in csvreader.fieldnames:
        qpylib.log(
            "ingest_feed_api.{0}: Update method is 'replace' so check for changes and update .".format(
                func_name
            ),
            level="info",
        )
        ...
        for k in QRADAR_RT_LIST:
            rt_title = "eiq_" + str(feed_id) + "_" + state["feed_name"] + "_" + k
            qpylib.log(
                "ingest_feed_api.{0}: Feed id {1}, add {2} observables into RT {3}.".format(
                    func_name, str(feed_id), len(data_to_add[k]), rt_title
                ),
                level="info",
            )
        ...
        qpylib.log(
            "ingest_feed_api.{0}: Update method is 'diff', so check for changes and update .".format(
                func_name
            ),
            level="info",
        )
        ...
        for k in QRADAR_RT_LIST:
            qpylib.log(
                "ingest_feed_api.{0}: Feed id {1}, remove {2} observables from RT {3}.".format(
                    func_name, str(feed_id), len(data_to_del), rt_title
                ),
                level="info",
            )
            rt_title = "eiq_" + str(feed_id) + "_" + state["feed_name"] + "_" + k
            ...
            for observable in data_to_del:
                qrdar_obj.observable_delete(
                    observable["table_name"], observable["outer_key"]
                )
                qpylib.log(
                    "ingest_feed_api.{0}: observable {1} deleted.".format(
                        func_name, observable["outer_key"]
                    ),
                    level="info",
                )
