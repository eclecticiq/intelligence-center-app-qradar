...
from app.collector.eiq_data import EIQApi, QradarApi, get_verify_ssl_value, get_proxy
...
def save_configuration():
    ...
    proxy_ip = str(form.get(PROXY_IP)).strip()
    proxy_username = str(form.get(PROXY_USERNAME)).strip()
    proxy_password = str(form.get(PROXY_PASSWORD)).strip()
    verify = str(form.get(VERIFY, ""))
    qpylib.log(form)
    ...
    if eiq_api_status_code not in [200, 201] or qradar_api_response not in [200, 201]:
        ...
        config[PROXY_PASSWORD] = Cipher(PROXY_PASSWORD, SHARED).encrypt(proxy_password)
...
def test_connection():
    ...
    api_key = str(form.get(API_KEY)).strip()
    qradar_security_token = str(form.get(SECURITY_TOKEN)).strip()
    verify = str(form.get(VERIFY, "")).strip()
    proxy_ip = str(form.get(PROXY_IP)).strip()
    proxy_username = str(form.get(PROXY_USERNAME)).strip()
    proxy_password = str(form.get(PROXY_PASSWORD)).strip()
    ...
    if eiq_api_status_code in [401]:
        config[STATUS_STRING] = (
            "EclecticIQ connectivity error. Status code: "
            + str(eiq_api_status_code)
            + " Wrong API key."
        )
        ...
        config[STATUS_STRING] = (
            "EclecticIQ connectivity error. Status code: "
            + str(eiq_api_status_code)
            + " Check EclectciIQ Intelligence Center url.."
        )
    elif qradar_api_response in [401]:
        config[STATUS_STRING] = (
            "QRadar connectivity error. Status code: "
            + str(qradar_api_response)
            + " Wrong API key."
        )
    elif qradar_api_response not in [200, 201]:
        config[STATUS_STRING] = "QRadar connectivity error. Status code: " + str(
            qradar_api_response
        )
    elif eiq_api_status_code not in [200, 201]:
        config[STATUS_STRING] = "EclecticIQ connectivity error. Status code: " + str(
            eiq_api_status_code
        )
    elif not missing_permissions and qradar_api_response == STATUS_CODE_200:
...
def lookup_observables():
    ...
    eiq_api = EIQ_api(
        baseurl=config_data["host"],
        eiq_api_version=config_data["version"][-2:],
        username="",
        password=config_data["api_key"],
        verify_ssl=get_verify_ssl_value(config_data.get(VERIFY)),
