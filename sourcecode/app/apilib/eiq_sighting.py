...
class EclecticIQ_api(object):
    ...
    def send_api_request(self, method, path, params=None, data=None, timeout=30):
        ...
        try:
            if method == 'post':
                ...
                LOG.error('EclecticIQ_api.{0}: Unknown method:{1}'.format(func_name, method))
                raise Exception
        except Exception:

            LOG.error('EclecticIQ_api.{0}: Could not perform request to EclecticIQ VA: {1}:{2}'.format(func_name, method, url))
        ...
        if r and r.status_code in [100, 200, 201]:
            ...
            try:
                ...
                LOG.error('EclecticIQ_api.{0}: EclecticIQ VA returned an error, code:[{1}], reason:[{2}], URL: [{3}], details:[{4}]'.format(
    ...
    def get_source_group_uid(self, group_name):
        func_name = sys._getframe().f_code.co_name
        LOG.info('%s: Requesting source id for specified group, name=["%s"]', func_name, group_name)
        r = self.send_api_request(
            'get',
        ...
        if not r.json()['data']:
            LOG.error('%s: Something went wrong fetching the group id. Please note the source group name is case sensitive! Received response: %s', func_name, str(r.json()))
            return "error_in_fetching_group_id"
        else:
            LOG.info('%s: Source group id received %s', func_name, str(r.json()))
            LOG.debug('%s: Source group id is: %s', func_name, str(r.json()['data'][0]['source']))
    ...
    def create_sighting(self, source, record):
        ...
        LOG.debug('%s: Creating sighting for record %s:%s', func_name, extract_kind, extract_value)
        ...
        r = self.send_api_request(
            'post',
            path=PATHS[self.eiq_version_path]['entities'],
            data=sighting)

        if r.status_code in [100, 200, 201]:
            LOG.info('%s: Sighting for record %s:%s has been created.', func_name, extract_kind, extract_value)
        else:
            LOG.info('%s: Sighting for record %s:%s has not been created.', func_name, extract_kind, extract_value)
...
eiq_tip=EclecticIQ_api(baseurl=eiq_url, eiq_version=eiq_version, username=eiq_user, password=eiq_password)
