...
class EIQApi:
    ...
    def create_sighting(
        ...
        sighting[DATA][DATA][TIME_STAMP] = format_time_to_iso(datetime.datetime.utcnow())
        sighting[DATA][DATA]["confidence"] = confidence_level
        sighting[DATA][DATA][TITLE] = title
        sighting[DATA][DATA][SECURITY_CONTROL]['time_start'] = format_time_to_iso(datetime.datetime(today.year, today.month, today.day, 0, 0, 0))
        sighting[DATA][META_KEY][TAGS_KEY] = tags
        sighting[DATA][META_KEY][INGEST_TIME_KEY] = format_time_to_iso(datetime.datetime.utcnow())
    ...
    def get_outgoing_feeds_ids_names(feeds):
        ...
        ids_and_names = []
        for data in feeds:
            if (data["content_type"] == "urn:eclecticiq.com:csv-extracts:1.0") and (data["transport_type"] == "eiq.outgoing-transports.http-download"):
